...
#	plt.show()
	...
	if max_value < 380:
		...
		error = width/2 - x
	error_idx = [error, data[y, x, 0], data[y, x, 1], data[y, x, 2]]
	return error_idx
...
if __name__ == '__main__':	
	...
	while True:
		try:
			...
			data = np.empty((height, width, 3), dtype=np.uint8)
			camera.capture(data, 'rgb')
			error_idx = red_distance(data, width, height, Rn, Gn, Bn)
			error = error_idx[0]
			if error == False and error_prime < 0:
				...
				error_prime = error
				sum_R = sum_R +  error_idx[1]
				sum_G = sum_G + error_idx[2]
				sum_B = sum_B + error_idx[3]
				Rn = sum_R/i
				Gn = sum_G/i
				Bn = sum_B/i
				i = i + 1
