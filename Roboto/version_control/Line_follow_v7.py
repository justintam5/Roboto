...
##
	...
	mask_mean = maskx*x
	mean_pos = (mask_mean.sum())
##
	cv2.imshow('HELLO TRAVLLER!!!!', data)
	cv2.imshow('HELLO AGAIN!!!!!!!!', mask)

	cv2.imwrite('{}/image_real_{}.png'.format(path, n), data)
	cv2.imwrite('{}/image_mask_{}.png'.format(path, n), mask)
	#data_file.write('{}, n: {}\n'.format(str(maskx), n))
	with open('{}/data_file.csv'.format(path), 'a', newline='') as myfile:
